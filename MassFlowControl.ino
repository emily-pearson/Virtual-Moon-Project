...
void loop() {
  ...
  if (Serial.available() > 0) {
    serial_flow_rate = Serial.read();
    Serial.write(serial_flow_rate);
  }

  
float duty_cycle = set_flow_rate * (1.023); // convert flow rate to duty cycle
unsigned int duty_cycle_round = round(duty_cycle); // round up for better replication of desired flow rate
Serial.println(duty_cycle_round);
Timer1.pwm(PWM_PIN,duty_cycle_round); 
